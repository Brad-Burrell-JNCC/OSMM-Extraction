# Name: GZpath_from_OSMM_BoundaryLine_CountryBorder_intersect.py
#       J:\GISprojects\Ordnance Survey MasterMap\Derived Data\Python\GZpath_from_OSMM_BoundaryLine_CountryBorder_intersect.py
#
# Author: Stuart Wallace - 2 Sept 2019
#
# Description: make a shapefile of coverage polygons for every .gz in the entire OSMM dataset...
#              ...intersect the coverage shapefile with the 2015 BoundaryLine CountryBorder linework...
...
if len(sys.argv) < 2:
	#sys.argv[1:] = ["-h"]
	print ' '
	print '  this script needs the root path of the OSMM gz file folder, ie > GZpath_from_OSMM_BoundaryLine_CountryBorder_intersect.py "J:\Reference\BaseMapping\OSMasterMap\\2019 complete GB dataset"'
	sys.exit(0)
...
# use SHUTIL to copy the default empty gdb to a new empty gdb (only if the .gdb doesn't already exist).
...
featureName = 'polygonFromGZ_BL_' + dts[2] + '_' + dts[1] + '_' + dts[4]
if arcpy.Exists(featureName):
	...
	featureName = 'polygonFromGZ_BL_' + dts[2] + '_' + dts[1] + '_' + dts[4] + '_' + hmsIndex[0] + '_' + hmsIndex[1] + '_' + hmsIndex[2]
	print featureName
...
# intersect the gz polygons with an existing BoundaryLine_CountryBorder feature to find only those gz squares that likely contain border polylines.
inFeatures = [featureName, workF + 'data\GDB\OSMM_MHW_feature_polylines_2018.gdb\OSMM_BL_CountryBorder_polyline_2018_BNG']
intersectFeatureName = 'gzPolygon_country_borders_intersect_BL_' + dts[2] + '_' + dts[1] + '_' + dts[4]
...
filename = workF + "Python\Output" + os.sep + 'GZpath_from_BoundaryLine_CountryBorder_' + dts[4] + '_' + dts[2] + '_' + dts[1] + '.txt'
